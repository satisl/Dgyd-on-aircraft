...
dst_path = r'D:\Double-digit-yolo-detection-on-aircraft\datasets\detect\origin'
...
for file in os.listdir(img_path1):
    ...
    with open(f'{txt_path2}/{txt_file}', 'w') as f:
        for cls, polygon in zip(clss, polygons):
            if len(polygon) > 0:
                polygon_np = np.array([(float(x) * width, float(y) * height) for x, y in polygon]).astype(int)
