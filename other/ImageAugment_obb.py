...
def parse_polygon(file, height, width):
    ...
    with open(file, mode="r") as f:
        for line in f.readlines():
            ...
            polygons.append(
                [
                    (float(infos[i]) * width, float(infos[i + 1]) * height)
...
def main(files, img_path1, img_path2, txt_path1, txt_path2, lock):
    for file in files:
        ...
        polygons = [
            ia.Polygon(polygon, label=cls) for polygon, cls in zip(polygons, clss)
        ]
        cnt = 0
        while cnt < aug_num:
            img_aug, polygons_aug = seq(images=[img], polygons=[polygons])
            height_aug, width_aug = img_aug[0].shape[:2]
            with open(f"{txt_path2}/{prefix}-{cnt}.txt", mode="w") as f:
                for polygon in polygons_aug[0]:
                    (x1, y1), (x2, y2), (x3, y3), (x4, y4) = [
                        [x / width_aug, y / height_aug] for (x, y) in polygon.exterior
                    ]
                    f.write(
                        f"{polygon.label} {x1} {y1} {x2} {y2} {x3} {y3} {x4} {y4}\n"
                    )
...
org_path = r"D:\Double-digit-yolo-detection-on-aircraft\datasets\obb\splited1"
dst_path = r"D:\Double-digit-yolo-detection-on-aircraft\datasets\obb\for trainning1"
