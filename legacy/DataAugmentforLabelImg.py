...
class DataAugmentForObjectDetection():
    ...
    def _addNoise(self, img):
        ...
        return random_noise(img, mode='gaussian', rng=int(time.time()), clip=True) * 255
...
def main(files,source_img_path, source_xml_path, save_img_path, save_xml_path):
    for file in files:
        cnt = 0
        pic_path = os.path.join(source_img_path, file)
        xml_path = os.path.join(source_xml_path, file.rsplit('.', 1)[0] + '.xml')
...
if __name__ == '__main__':
    ...
    dataAug = DataAugmentForObjectDetection(rotation_rate=0.5, max_rotation_angle=5,
                                            crop_rate=0.5, shift_rate=0.5, change_light_rate=0.5,
                                            add_noise_rate=0.5, flip_rate=0.5,
                                            cutout_rate=0.5, cut_out_length=50, cut_out_holes=1, cut_out_threshold=0.5,
                                            is_addNoise=True, is_changeLight=True, is_cutout=False,
                                            is_rotate_img_bbox=True,
                                            is_crop_img_bboxes=True, is_shift_pic_bboxes=True,
                                            is_filp_pic_bboxes=False)  # 数据增强工具类

    toolhelper = ToolHelper()  # 工具

    org_path = r'D:\Double-digit-yolo-detection-on-aircraft\datasets\5/splited1'
    dst_path = r'D:\Double-digit-yolo-detection-on-aircraft\datasets\5/for trainning1'
    os.makedirs(dst_path, exist_ok=False)
    lock = threading.Lock()

    for i in ['train', 'val', 'test']:
        # 获取相关参数
        source_img_path = f'{org_path}/images/{i}'  # 图片原始位置
        source_xml_path = f'{org_path}/annotations/{i}'  # xml的原始位置

        save_img_path = f'{dst_path}/images/{i}'  # 图片增强结果保存文件
        save_xml_path = f'{dst_path}/annotations/{i}'  # xml增强结果保存文件

        os.makedirs(save_img_path, exist_ok=False)
        os.makedirs(save_xml_path, exist_ok=False)
        img_paths = os.listdir(source_img_path)
        img_paths.sort()
        split_num = len(img_paths) // worker_num
        tasks = [threading.Thread(target=main, args=(
            img_paths[i * split_num:(i + 1) * split_num], source_img_path, source_xml_path, save_img_path,
            save_xml_path))
                 for i in
                 range(worker_num)]
        for i in tasks:
            i.start()
        if worker_num * split_num < len(img_paths):
            t = threading.Thread(target=main, args=(
            img_paths[worker_num * split_num:], source_img_path, source_xml_path, save_img_path, save_xml_path))
            t.start()
            t.join()
        for i in tasks:
            i.join()
