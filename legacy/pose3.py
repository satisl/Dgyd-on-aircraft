...
cap_path = './benchmark/record1.mp4'
camera_frequence = 1 / 60  # 每几秒读取work_num_1帧

model_7_path = './yolov8/pose_400dataset_imgsz640_v8n_SGD/weights/best.pt'
...
class Detect1(object):
    def detect(self, queue1, queue2, timeout):
        model1 = YOLO(model_7_path, task='pose')
        ...
        while flag:
            try:
                ...
                xywhs, xys = self.predict(imgsz1, model1, frame, conf, iou)
                height, width = frame.shape[:2]

                # 截取并旋转靶子图像
                cropped_imgs = self.crop_and_rotate(xywhs, xys, frame, width, height)

                # 靶子检测结果画框
                for (x, y, w, h), (px, py) in zip(xywhs, xys):
    ...
    def predict(self, imgsz, model, img, conf, iou):
        ...
        xys = [_.tolist() for _ in r.keypoints.xy][0]
        return xywhs, xys

    def crop_and_rotate(self, xywh, xy, image, width, height):
        rotated_imgs = []
        for (x, y, w, h), (px, py) in zip(xywh, xy):
            # 截取长宽
            ...
            rot_mat = cv2.getRotationMatrix2D((x - left, y - top), angle_deg, 0.9)
...
class Detect2(object):
    def detect(self, queue1, queue2, queue3, timeout):
        ...
        while flag:
            try:
                frame, xywhs, rotated_imgs = queue1.get(timeout=timeout)
                # 识别旋转后图片上两单位数的数值，并合并为双位数数值
                double_digits, xywhs = self.predict(imgsz2, model2, rotated_imgs, xywhs, conf, iou)
                ...
                if len(rotated_imgs) > 0:
                    ...
                    if target_num > target_saving_per_frame:
                        queue3.put((double_digits, rotated_imgs))
                        target_num = 0
                lock.release()
