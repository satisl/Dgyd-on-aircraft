...
model_7_path = './yolov8/segment_400dataset_imgsz640_v8n_SGD/weights/best.pt'
...
class Detect1(object):
    def detect(self, queue1, queue2, timeout):
        model1 = YOLO(model_7_path, task='segment')
        ...
        while flag:
            try:
                ...
                xywhrs, xyxys, xyxyxyxys = self.predict(imgsz1, model1, frame, conf, iou)
                ...
                queue2.put((frame, xyxyxyxys, rotated_imgs))
    ...
    def predict(self, imgsz, model, img, conf, iou):
        ...
        if r.masks is not None:
            ...
            for xy in xys:
                ...
                (cx, cy), (w, h), r = rect
                xywhrs.append([cx, cy, w, h, r])

                (x1, y1), (x2, y2), (x3, y3), (x4, y4) = cv2.boxPoints(rect).tolist()
                xyxyxyxys.append([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])

                left = min(x1, x2, x3, x4)
                right = max(x1, x2, x3, x4)
                top = min(y1, y2, y3, y4)
                bottom = max(y1, y2, y3, y4)
                xyxys.append([left, top, right, bottom])
        return xywhrs, xyxys, xyxyxyxys
...
class Detect2(object):
    def detect(self, queue1, queue2, queue3, timeout):
        ...
        while flag:
            try:
                frame, xyxyxyxys, rotated_imgs = queue1.get(timeout=timeout)
                # 识别旋转后图片上两单位数的数值，并合并为双位数数值
                double_digits, xyxyxyxys = self.predict(imgsz2, model2, rotated_imgs, xyxyxyxys, conf, iou)
