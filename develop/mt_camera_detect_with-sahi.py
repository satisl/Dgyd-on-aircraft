...
model_456_path = r'D:\yolov8\yolov8\456_300dataset_imgsz640_v8n_SGD\weights\best.pt'
...
def detect_456(model_456, img):
    results_456 = get_sliced_prediction(img, model_456, slice_height=slice_height,
                                        slice_width=slice_width, overlap_width_ratio=0.2,
                                        overlap_height_ratio=0.2, verbose=True)

    # 获取图片检测相关信息
    digits = []
    locaters = []
    for i in results_456.object_prediction_list:
        if i.category.id == 0:
            locaters.append(i.bbox.to_xywh())
        else:
            digits.append(i.bbox.to_xywh())
    digits = [(i[0] + i[2] / 2, i[1] + i[3] / 2, i[2], i[3]) for i in digits]
    locaters = [(i[0] + i[2] / 2, i[1] + i[3] / 2, i[2], i[3]) for i in locaters]
...
if __name__ == '__main__':
    ...
    # 视频流队列
    input_queues = [queue.Queue(maxsize=5) for i in range(worker_num)]
    ...
    models = [(AutoDetectionModel.from_pretrained(
        model_type='yolov8', model_path=model_456_path, confidence_threshold=0.5,
        image_size=imgsz1, device='0'),
               YOLO(model_789_path, task='detect'))
        for i in range(worker_num)]
